...
def capture_lead(s,d):
    ...
    if not existing:
        ...
        if to:queue(s,'lead-'+key,email(to),'Novo diagnóstico regulatório solicitado',json.dumps(result,ensure_ascii=False,indent=2))
    return {'ok':True,'request_id':key,'message':'Solicitação registrada. A equipe da LCF Consulting avaliará o contexto para a conversa inicial.'}
...
def subscribe(s,d,user=None):
    ...
    if not os.environ.get('SMTP_HOST') and os.environ.get('MONITOR_DEV')!='1':raise Problem(503,'Os alertas ainda não estão disponíveis. Solicite um diagnóstico para definir seu acompanhamento.')
    ...
    queue(s,'confirm-'+key,address,'Confirme seus alertas de regulação de IA',f'Você solicitou alertas do Monitor Legislativo de IA. Confirme em {site}/alertas/#confirm={token}\nO link expira em 7 dias. Se não solicitou, ignore. Nenhum alerta será enviado sem confirmação.',account)
